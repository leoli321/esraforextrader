...
async def scalp(order_mode=False):
    ...
    for market in symbols:
        ...
        if (overallscore['buy_signal'] > long_buy and overallscore['sell_signal'] < long_sell):
            ...
            buyCurrency(market, 'short', 10000.0) #10 refers to amount of money wanted to invest
...
def buyCurrency(market, positionType, positionSize): #positionSize in Dollars (Amount of Money Plan on Investing)
    if (positionType == 'long'):
        ...
        rv = oanda.request(r)
        bidding_price = float(r.response['prices'][0]['bids'][0]['price']);
        buy_units = (int) (positionSize / bidding_price)
        
        #Expiry Time for 15 minutes (Not Used)
        current_time = datetime.datetime.utcnow()
        added_time = current_time.minute + 15
        if (added_time > 59):
            added_time -= 60
        trade_expire = datetime.datetime(current_time.year, current_time.month, current_time.day, current_time.hour, added_time,  current_time.second)
        ...
        # limitOrderData = {
        #     "order": {
        #         "price": round(bidding_price * 1.001, 3),
        #         "stopLossOnFill": {
        #             "timeInForce": "GTC",
        #             "price": round(bidding_price * .995, 3)
        #         },
        ...
        if (r2.response.orderCancelTransaction):
            print("_______LONG ORDER HAS NOT BEEN FULFILLED_______")
            print("Reason: " + r2.response.orderCancelTransaction.reason)
            return
        print("\n")
        print("_______LONG ORDER FULFILLED______")
        print("\n")
        ...
        short_units = (int) (positionSize / ask_price)
...
#client sentiment strategy
async def client_sentiment(market):
    enter_signal = 0.0
    exit_signal = 0.0
    ncs = 0
    ncl = 0
    params = {"instrument": market}
    req = labs.CommitmentOfTraders(params=params)
    res = oanda.request(req)
    for i in range(len(res[market])):
        ncs += float(res[market][i]["ncs"])
        ncl += float(res[market][i]["ncl"])
    ncl = ncl/len(res[market])
    ncs = ncs/len(res[market])
    ncl_ratio = ncl/(ncl+ncs)
    ncs_ratio = ncs/(ncl+ncs)
    if ncl_ratio >= .67:
        enter_signal += 10*ncl_ratio
    elif ncs_ratio >= .67:
        exit_signal += 10*ncs_ratio
    return {
        "enter_signal": enter_signal,
        "exit_signal": exit_signal
    }
...
loop.run_until_complete(runforextrader(loop))
